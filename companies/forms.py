...
class WorkPracticeTestForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['name', 'duration', 'attempts_allowed', 'questions_count', 'pass_percentage', 'start_time', 'end_time', 'is_active']
        widgets = {
            ...
            'pass_percentage': forms.NumberInput(attrs={'class': 'w-full h-[52px] px-4 rounded-[14px] border border-slate-200 bg-white text-slate-900 text-[15px] placeholder:text-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/15 transition-all duration-200 hover:border-slate-300 outline-none shadow-sm', 'min': 1, 'max': 100}),
